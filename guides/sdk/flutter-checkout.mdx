...
| `language` | `YunoLanguage` | No | Device locale | UI language |
| `cardFlow` | `CardFlow` | No | `oneStep` | `oneStep` or `multiStep` card entry |
...
For reactive architectures (BLoC, Riverpod), listen to the payment event stream:
...
The SDK handles 3D Secure authentication automatically on both platforms. No additional code is required. The payment result includes the final outcome after 3DS completes.
