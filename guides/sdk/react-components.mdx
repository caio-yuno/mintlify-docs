...
description: "Embed Yuno checkout in React applications with pre-built components, hooks, and TypeScript support"
---
...
There is also an unofficial community React wrapper (`@latiscript/yuno-react`) available for React 16.8+ with Full and Lite checkout modes.
...
## Unofficial React wrapper

The community-maintained `@latiscript/yuno-react` package provides an alternative React integration with:

- Support for React 16.8+
- Context-based state management via `YunoProvider` and `useYuno` hook
- `Full` and `Lite` checkout components
- `PayButton` component

```tsx
import { YunoProvider, Full, useYuno } from '@latiscript/yuno-react';
